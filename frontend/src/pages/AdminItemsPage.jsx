...
const AdminItemsPage = () => {
  ...
  const statusOptions = [
    ...
    { value: 'reserved', label: 'Menunggu ACC', class: 'bg-orange-500/20 text-orange-600' }, // ✅ TAMBAHIN INI BROK
              ...
              <table className="w-full min-w-[800px]">
                <thead className={theadClass}>
                  <tr className="text-left">
                    ...
                    <th className={thClass}>Stok</th> {/* ✅ TAMBAH KOLOM STOK */}
                  ...
                  {items.map((item) => {
                    ...
                    return (
                      <tr key={item.id} className={`border-t ${isDark ? 'border-white/5' : 'border-gray-100'} hover:bg-white/5 transition`}>
                        ...
                        <td className={`${tdClass} ${textClass} font-bold`}>{item.total_stock || 1} Pcs</td> {/* ✅ TAMPILIN STOK */}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm overflow-y-auto" onClick={() => setShowModal(false)}>
          <div className={`${cardClass} max-w-lg w-full p-6 rounded-2xl space-y-4 max-h-[90vh] overflow-y-auto`} onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {/* ✅ GRID BARU: Bikin Nama Barang dan Stok Sebelahan */}
              <div className="grid grid-cols-3 gap-4">
                ...
                <div className="col-span-1">
                  <label className={`block text-sm font-medium mb-1 ${isDark ? 'text-slate-300' : 'text-gray-700'}`}>Total Stok *</label>
                  <input type="number" min="1" max="999" value={formData.total_stock} onChange={(e) => setFormData({...formData, total_stock: parseInt(e.target.value) || 1})} className={`w-full px-4 py-2 rounded-xl border outline-none transition ${inputClass}`} required />
                </div>
              </div>
              ...
              {/* BARIS BARU UNTUK SYARAT JAMINAN & SURAT */}
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className={`block text-sm font-medium mb-1 ${isDark ? 'text-slate-300' : 'text-gray-700'}`}>Syarat Jaminan ID</label>
                  <select value={formData.required_id} onChange={(e) => setFormData({...formData, required_id: e.target.value})} className={`w-full px-4 py-2 rounded-xl border outline-none transition ${inputClass}`}>
                    <option value="none">Tidak Ada</option>
                    <option value="ktm">Wajib Foto KTM</option>
                    <option value="ktp">Wajib Foto KTP</option>
                  </select>
                </div>
                <div>
                  <label className={`block text-sm font-medium mb-1 ${isDark ? 'text-slate-300' : 'text-gray-700'}`}>Syarat Surat Izin</label>
                  <select value={formData.require_letter} onChange={(e) => setFormData({...formData, require_letter: e.target.value === 'true'})} className={`w-full px-4 py-2 rounded-xl border outline-none transition ${inputClass}`}>
                    <option value="false">Tidak Perlu</option>
                    <option value="true">Wajib Upload PDF</option>
                  </select>
                </div>
              </div>
